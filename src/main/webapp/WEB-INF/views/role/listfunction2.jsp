...
<%@ page import="com.ygr.usermanage.model.Function"%>
...
<body>
	...
	<p>
		[<a href="listrole.do">返回</a>]
	</p>
	
	<span style="color: red">角色名称：${role.rolename}${mmmmm}</span>
			...
			<c:forEach items="${functionAllLists}" var="x">
				<tr>
					...
					<c:forEach items="${functionLists}" var="y">
						...
						<c:if test="${y.function_id eq x.function_id}">
							<%
								z = 1;
							%>
						</c:if>
					...
					<%
						if (z == 1) {
					%>
					<td bgcolor="#ffffff"><input name="selectId" type="checkbox"
						value="${x.function_id}" checked="checked" /></td>
					<%
						} else {
					%>
					<td bgcolor="#ffffff"><input name="selectId" type="checkbox"
						value="${x.function_id}" /></td>
					<%
						}
					%>
